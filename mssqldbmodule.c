...
#define _MSSQL_MODULE_VERSION "1.1.0"
...
static PyObject *_mssql_select_db(_mssql_connection *self, PyObject *args) {
    ...
    if ((dbname[0] == '[') && dbname[strlen(dbname)-1] == ']')
        ...
        snprintf(command, 255, "USE [%s]", dbname);
        Py_XDECREF(value); Py_XDECREF(search); Py_XDECREF(replace);
        Py_XDECREF(replaced);
    }
    ...
    /* Do not use Py_BEGIN_ALLOW_THREADS here; doing so causes severe
       unstability in multithreaded scripts (especially when select_db is called
       from _mssql_connect). Any number of threads > 1 causes problems.
       It has something to do with msg_handler. */
...
Quoting means transforming data into form suitable for putting\n\
in a query string.\n\nExamples:\n\
...
int _mssql_query_timeout_set(_mssql_connection *self, PyObject *val, void *closure) {
    ...
    if (val == NULL) {
        PyErr_SetString(PyExc_TypeError, "Cannot delete 'query_timeout' attribute.");
        return -1;
    }
...
int err_handler(DBPROCESS *dbproc, int severity, int dberr, int oserr,
    ...
    fprintf(stderr, "\n*** err_handler(dbproc = %p, severity = %d, dberr = %d, "
            ...
            (void *)dbproc, severity, dberr, oserr, dberrstr, oserrstr, DBDEAD(dbproc));
    fprintf(stderr, "*** previous max severity = %d\n\n", _mssql_last_msg_severity);
    fflush(stderr);
#endif

    // mute if below the acceptable threshold
    o = PyObject_GetAttrString(_mssql_module, "min_error_severity");
    lo = PyInt_AS_LONG(o);
    Py_DECREF(o);
...
int msg_handler(DBPROCESS *dbproc, DBINT msgno, int msgstate, int severity,
    ...
    fprintf(stderr, "\n+++ msg_handler(dbproc = %p, msgno = %d, msgstate = %d, "
            ...
            (void *)dbproc, msgno, msgstate, severity, msgtext, srvname, procname, line);
    fprintf(stderr, "+++ previous max severity = %d\n\n", _mssql_last_msg_severity);
    fflush(stderr);
#endif
...
#ifdef PYMSSQLDEBUG
    if (n == NULL)
        fprintf(stderr, "msg_handler - list node dbproc %p not found\n", dbproc);
#endif
...
static PyObject *_mssql_connect(_mssql_connection *self, PyObject *args,
    ...
    PyObject *ologintimeout, *o, *dbnameo;
    ...
    clr_err(NULL);  // clear global errors
    ...
    *dbconn->charset = 0;
    ...
    /* Do not use Py_BEGIN_ALLOW_THREADS here; doing so causes severe
       unstability in multithreaded scripts. Any number of threads > 1
       causes problems. I suspect a locking problem in dbopen() -- it may
       contain a race condition in an internal list which is unprotected
       from concurrent access. */

    dbconn->dbproc = dbopen(login, server);
    DBFREELOGIN(login);       // Frees the login record, can be called immediately after dbopen

    if (dbconn->dbproc == NULL) {
        connection_object_list = connection_object_list->next;
        PyMem_Free(n);
        Py_DECREF(dbconn);
        if (maybe_raise_MssqlDatabaseException(NULL)) // we hope it will raise something
            return NULL;
    ...
    if (database) {
        dbnameo = PyString_FromString(database);
        o = _mssql_select_db(dbconn, dbnameo);
        Py_XDECREF(dbnameo);
...
#if PY_MAJOR_VERSION >= 3
/* module definition */
static PyModuleDef _mssqlmodule = {
    PyModuleDef_HEAD_INIT,
    "_mssql",
    "Low level module to connect to MS-SQL Servers.",
    -1,
    NULL, NULL, NULL, NULL, NULL
};
#endif
...
PyInit_mssql(void) {
    ...
    // add version number
    if (PyModule_AddStringConstant(_mssql_module, "__version__", _mssql_module_version) == -1) return;
...
PyObject *format_and_run_query(_mssql_connection *self, PyObject *args) {
    RETCODE rtc;
    char *query;
    PyObject *format = NULL, *params = NULL, *tuple;
    ...
    Py_INCREF(format);  // PyArg_ParseTuple() returns BORROWED ref...
    ...
    if (params != NULL) {
        ...
        Py_DECREF(tuple);    // also format and params
        if (formatted == NULL)  return NULL;
        format = formatted;  // format not needed anymore, DECREFed above
    }
    ...
    dbcmd(self->dbproc, query);       // prepare the query buffer
    Py_XDECREF(format);               // this also frees 'query'

    db_sqlexec(rtc, self->dbproc);    // execute the query
    check_cancel_and_raise(rtc, self);

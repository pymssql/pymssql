...
class pymssqlCursor(object):
	...
	def rowcount(self):
		...
		"""
		return self._source._cnx.rows_affected
	...
	def lastrowid(self):
		...
		return self._source._cnx.identity
	...
	def execute(self, operation, *args):
		...
		try:
			if args == ():
				self._source._cnx.execute_query(operation)
			else:
				self._source._cnx.execute_query(operation, args[0])

			self.description = self._source._cnx.get_header()
	...
	def nextset(self):
		...
		"""
		try:
			if self._source._cnx.nextresult():
				self._rownumber = 0
				self.description = self._source._cnx.get_header()
				return 1
		except _mssql.MssqlDatabaseException, e:
	...
	def fetchone(self):
		...
		"""
		if self._source._cnx.get_header() == None:
			raise OperationalError, "No data available."

		try:
			if self.as_dict:
				row = iter(self._source._cnx).next()
				self._rownumber += 1
				return row
			else:
				row = iter(self._source._cnx).next()
	...
	def fetchall(self):
		...
		try:
			if self.as_dict:
				list = [ row for row in self._source._cnx ]
			else:
				list = [tuple([row[r] for r in sorted(row.keys()) if type(r) == int]) for row in self._source._cnx]
	...
	def fetchall_asdict(self):
		...
		try:
			return [ row for row in self._source._cnx ]
		except _mssql.MssqlDatabaseException, e:
			raise OperationalError, e[0]
		except _mssql.MssqlDriverException, e:
			raise InterfaceError, e[0]
...
class pymssqlCnx:
	...
	def cursor(self):
		...
		"""
		return pymssqlCursor(self, self.as_dict)
